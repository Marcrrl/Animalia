...
public class AnimaliaApplication {
	...
	@Bean
	CommandLineRunner initData(AnimalesRepositorio animalesRepositorio, UsuarioRepositorio usuariosRepositorio, BCryptPasswordEncoder passwordEncoder) {
		return (args) -> {
			if (animalesRepositorio.count() == 0 && usuariosRepositorio.count() == 0) {
				...
				List<Usuarios> usuarios = Arrays.asList(
						new Usuarios(null, "Juan", "Pérez", "juan.perez@example.com", passwordEncoder.encode("123"), "123456789", "Calle Falsa 123", "bardockNegro+.jpg", Usuarios.TipoUsuario.USER, LocalDate.now(), 0),
						new Usuarios(null, "Ana", "García", "ana.garcia@example.com", passwordEncoder.encode("123"), "987654321", "Avenida Siempre Viva 456", "", Usuarios.TipoUsuario.ADMIN, LocalDate.now(), 10),
						new Usuarios(null, "Carlos", "Hernández", "carlos.hernandez@example.com", passwordEncoder.encode("123"), "1122334455", "Calle Luna 789", null, Usuarios.TipoUsuario.USER, LocalDate.now(), 5),
						new Usuarios(null, "Lucía", "Martínez", "lucia.martinez@example.com", passwordEncoder.encode("123"), "5566778899", "Avenida Sol 321", "", Usuarios.TipoUsuario.USER, LocalDate.now(), 8),
						new Usuarios(null, "Pedro", "Gómez", "pedro.gomez@example.com", passwordEncoder.encode("123"), "9988776655", "Calle Estrella 654", null, Usuarios.TipoUsuario.ADMIN, LocalDate.now(), 15));
